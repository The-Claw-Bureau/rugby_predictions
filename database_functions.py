...
def notifyTelegram(bot_token, chat_id, ALLOWED_USERS, summary=None, message=None, image_path=None):
    if str(chat_id) not in ALLOWED_USERS:
        logging.warning("🚫 Unauthorized user attempted to access bot.")
        return
    ...
    if image_path:
        ...
        with open(image_path, 'rb') as photo:
            files = {"photo": photo}
            data = {"chat_id": chat_id, "caption": summary or ""}
            response = requests.post(url, files=files, data=data)

        if response.status_code == 200:
            logging.info("🖼️ Telegram image sent successfully.")
        else:
