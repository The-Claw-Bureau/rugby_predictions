...
# Function to wrap custom losses
class CustomLossWrapper(tf.keras.losses.Loss):
    def __init__(self, loss_function, lambda_sign=2.0, name="custom_loss"):
...
def run_experiment(response_variable_name, transform_target, original_df,
...
#             y_pred_labels = np.argmax(y_pred, axis=1) if y_pred.ndim > 1 else y_pred
    ...
    validation_results, original_df_wProbabilities = get_validation_success(response_variable_name, transform_target, model_type, original_df, original_X, model, scaler, pca_enabled, pca_threshold, pca, outlier_dict, outlier_detection, validation_dict)
...
def find_optimal_general_parameters(response_variable_name, transform_target, original_df, all_features_df, model_type, X, y, retrying_optimal_iterations, model_ass_val, model_ass_val_direction, pcas, pca_thresholds, learning_rates, batch_sizes, optimizers, loss_functions, metrics, epochs, activation_layers_list, n_splits, class_weights_enabled, outlier_dicts, stratification_enabled, lr_schedule_enabled, lr_schedule_strategies, dropout_enabled, dropout_rates, early_stopping_metrics, model_layer, validation_dict):
    

    if os.path.exists('optimal_parameters_df.csv'):
        with open('optimal_parameters_df.csv', 'r') as f:
            optimal_parameters_df = f.read()
            print(f"{'optimal_parameters_df.csv'} exists")
        optimal_parameters_df = pd.read_csv('optimal_parameters_df.csv')

    
    else:
        optimal_parameters_df = pd.DataFrame(columns = ['model_layer', 'fine_tuning', 'iteration_num'])

    for loop_num in range(0, retrying_optimal_iterations):
        
        print("'''''''' Getting optimal parameters ''''''''")
        print("''''''''", retrying_optimal_iterations)
        print('')
        ...
        if len(results_df) == 0:
            ...
            optimal_parameters_df.to_csv('optimal_parameters_df.csv', index = False)
...
#             results_df, folds_df, feature_importances_df = iterate_through_model_variations(model_ass_val, model_ass_val_direction, [model_to_use['pca']], [model_to_use['pca_threshold']], [model_to_use['learning_rate']], [model_to_use['batch_size']], [model_to_use['optimizer']], [model_to_use['loss_function']], metrics, epochs, [model_to_use['activation_layers']], n_splits, [model_to_use['class_weights_enabled']], [model_to_use['outlier_dict']], [model_to_use['stratification_enabled']], lr_schedule_enabled, lr_schedule_strategies, [model_to_use['dropout_enabled']], [model_to_use['dropout_rate']], [model_to_use['early_stopping_metric']], validation_dict = validation_dict)
            ...
            lr_schedule_enabled = [results_df.sort_values(model_ass_val, ascending = model_ass_val_direction).iloc[0]['lr_schedule_enabled']]
            lr_schedule_strategies = [results_df.sort_values(model_ass_val, ascending = model_ass_val_direction).iloc[0]['lr_schedule_strategy']]
            lr_schedule_strategies = replace_df_objects_with_functions(lr_schedule_strategies)
            lr_schedule_strategies = lr_schedule_strategies.replace(': nan,', ': None,')
            lr_schedule_strategies = [eval(lr_schedule_strategies)]

            lr_schedule_strategy

            print(lr_schedule_strategies, 'lr_schedule_strategies')
        ...
        lr_schedule_strategy = top_model['lr_schedule_strategy']
        lr_schedule_strategy = replace_df_objects_with_functions(lr_schedule_strategy)
        lr_schedule_strategy = lr_schedule_strategy.replace(': nan,', ': None,')
        lr_schedule_strategy = [eval(lr_schedule_strategy)]

        top_model['lr_schedule_strategy'] = lr_schedule_strategy

        print('lr_schedule_strategy', lr_schedule_strategy)
...
#         print(mean, std, original_df_to_use[['prediction', response_variable_name]])
        ...
        if pd.notna(original_df_to_use['prediction_error']):
            ...
            original_df_to_use['prediction_error'] = None
            original_df_to_use['prediction_sign_success'] = None
...
def save_model_and_attributes(folder_name, model_name, top_model, X):
